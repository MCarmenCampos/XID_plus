...
class posterior_sed():
    def __init__(self, fit, priors,sed_prior_model,scale=False):
...
def swinbank_templates(SPIRE=True, PACS=True, MIPS=True, LABOCA=True):
    ...
    import pandas as pd
    #read in sw's templates. Units:  wavelength: [um]  //  S(nu)
    template = Table.read(xidplus.__path__[0]+'/../test_files/swinbank_templates/all_templates.csv')
    #create dataframe with column==wavelength [um]
