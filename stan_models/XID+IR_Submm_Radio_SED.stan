...
model{
  ...
  vector[npix_lb850] db_hat_alma850;//model of map
  ...
  vector[npix_lb850] sigma_tot_alma850;
 ...
 //Prior on conf
  ...
  for (k in 1:npix_lb850) {
    db_hat_lb850[k] <- bkg[7];
    sigma_tot_lb850[k]<-sqrt(square(sigma_lb850[k])+square(sigma_conf[7]));
  }
  for (k in 1:nnz_plw) {
    db_hat_lb850[Row_lb850[k]+1] <- db_hat_lb850[Row_lb850[k]+1] + Val_lb850[k]*src_f[Col_lb850[k]+1][7];
      }
  ...
  db_alma850 ~ normal(db_hat_lb850,sigma_tot_lb850);
