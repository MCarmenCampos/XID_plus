...
model{
 ...
 //Prior on conf
  ...
  for (i in 1:nsrc){
	...
	//print(f_tmp)
        ps[t]<-normal_lpdf(src_f[i]|f_tmp,0.3*f_tmp);//pow(10.0,Nbb[i])*interpolateLinear(SEDs_sig[t],z_median[i]*100.0));
    }
...
for (i in 1:nsrc){
	...
	for (b in 1:nband) {
        ...
        p_raw[t] = (1.0/nTemp)*exp(normal_lpdf(src_f[i]|f_tmp,0.3*f_tmp));//pow(10.0,Nbb[i])*interpolateLinear(SEDs_sig[t],z_median[i]*100.0)));
     }
     for (t in 1:nTemp){
