...
data
{
  ...
  real bkg_prior[7];//prior estimate of background
  real bkg_prior_sig[7];//sigma of prior estimate of background
  real conf_prior_sig[7];
  ...
  //----ALMA------
  int<lower=0> npix_lb870;//number of pixels
  int<lower=0> nnz_lb870; //number of non neg entries in A
  vector[npix_lb870] db_lb870;//flattened map
  vector[npix_lb870] sigma_lb870;//flattened uncertianty map (assuming no covariance between pixels)
  vector[nnz_lb870] Val_lb870;//non neg values in image matrix
  int Row_lb870[nnz_lb870];//Rows of non neg valies in image matrix
  int Col_lb870[nnz_lb870];//Cols of non neg values in image matrix
  //----VLA------
  real<lower=0.0> db_VLA[nsrc];
  real<lower=0.0> sigma_VLA[nsrc];
...
model{
  ...
  vector[npix_lb870] db_hat_alma870;//model of map
  ...
  vector[npix_lb870] sigma_tot_alma870;
 ...
 //Prior on conf
  ...
  for (k in 1:npix_lb870) {
    db_hat_lb870[k] <- bkg[7];
    sigma_tot_lb870[k]<-sqrt(square(sigma_lb870[k])+square(sigma_conf[7]));
  }
  for (k in 1:nnz_plw) {
    db_hat_lb870[Row_lb870[k]+1] <- db_hat_lb870[Row_lb870[k]+1] + Val_lb870[k]*src_f[Col_lb870[k]+1][7];
      }
  ...
  db_alma870 ~ normal(db_hat_lb870,sigma_tot_lb870);
  \\db_VLA ~ normal(src_f[8],sigma_VLA);
